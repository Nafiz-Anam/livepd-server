...
module.exports = async (req, res, next) => {
    ...
    if (validateIP(ip)) {
        ...
        qb.release();
        var updated_at = new Date().toJSON().substring(0, 19).replace('T', ' ');
        ...
        if (suspicious_email.includes(order_email)) {
            ...
            let qr_payment = await merchantOrderModel.selectOne("id", { order_no: req.bodyString('order_id') }, 'qr_payment')
            if(qr_payment){
                let qr_data = {
                    payment_status: 'Failed',
                    transaction_date:updated_at
                }
                merchantOrderModel.updateDynamic(qr_data, {id:qr_payment.id}, 'qr_payment')
