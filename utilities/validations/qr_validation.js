...
const qr_validation = {
    ...
    add: async (req, res, next) => {
        ...
        const schema = Joi.object().keys({
            ...
            total_collection: Joi.string().valid("per_day",'per_month','till_expiry').required().error(() => {
                return new Error("total collection is required.")
            }),
            start_date: Joi.string().allow(''),
            is_expiry: Joi.alternatives().conditional('type_of_qr', {
                is: "Dynamic_QR", then: Joi.number().valid(0, 1).required().error(() => {
                    return new Error("expiry is required.")
                }),
                otherwise: Joi.string().optional().allow("")
            }),
            end_date: Joi.alternatives().conditional('is_expiry', {
                is: 1, then: Joi.date().iso().min(Joi.ref('start_date')).required().error(() => {
                    return new Error("Invalid end date. End should be greater or equal to start date.")
                }),
                otherwise: Joi.string().optional().allow("")
            }),
            ...
            qty_frq: Joi.string().valid("per_day","per_month","till_expiry").required().error(() => {
    ...
    update: async (req, res, next) => {
        const schema = Joi.object().keys({
            ...
            start_date: Joi.date().iso().required().error(() => {
                return new Error("start date required.")
            }),
            is_expiry: Joi.number().valid(0, 1).required().error(() => {
                return new Error("expiry is required.")
            }),
            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (record_exist) {
                        ...
                        res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(!record_exist?'Record not found or already deactivated.':""));
                    }
                }
            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (record_exist) {
                        ...
                        res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(!record_exist?'Record not found or already activated.':""));
                    }
                }
            } catch (error) {
                res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(error));
        ...
        if (checkEmpty(req.body, ["qr_id"])) {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    let perDayData = await qrGenerateModule.selectOne({ 'qr_id': record_id}); 
                    if(perDayData.is_expiry == 1){
                    if(perDayData.qty_frq=='per_day'){
                        let day = new Date().toLocaleDateString('sv');
                        var sum_quantity = await qrGenerateModule.per_day_quantity({ 'payment_id': `'${perDayData.qr_id}'`,'currency': `'${perDayData.currency}'`},`'${day}'`,'qr_payment');
                    }
                    if(perDayData.qty_frq=='per_month'){
                        const d = new Date();
                        let month = d.getUTCMonth() + 1;
                        var sum_quantity = await qrGenerateModule.per_month_quantity({ 'payment_id': `'${perDayData.qr_id}'`,'currency': `'${perDayData.currency}'`},`'${month}'`,'qr_payment');
                    }
                    if(perDayData.qty_frq=='till_expiry'){
                        
                        let expiry_date = perDayData.end_date;
                        var sum_quantity = await qrGenerateModule.per_day_quantity({ 'payment_id': `'${perDayData.qr_id}'`,'currency': `'${perDayData.currency}'`},`'${expiry_date}'`,'qr_payment');
                    }
                }
                    let record_exist = await checkifrecordexist({ 'qr_id': record_id, 'is_reseted': 0, 'is_expired': 0 }, 'merchant_qr_codes');
                    let deactivate_data = await checkifrecordexist({ 'qr_id': record_id, "status": 0 }, "merchant_qr_codes")
                    // let record_reset = await checkifrecordexist({ 'id': record_id, 'is_reseted': 0 }, 'merchant_qr_codes');
                    if (!record_exist || !deactivate_data || sum_quantity >= perDayData.overall_qty_allowed) {
                        if (!record_exist) {
                            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Record not found`)); 
                     }
                     else if (!deactivate_data) {
                         res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Invalid link`)); 
                      }
                      else if (sum_quantity >= perDayData.overall_qty_allowed) {
                         res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`You can not make payment for this link.Maximum quantity reached`)); 
                      }
                    } else {
        ...
        let email = req.body.emails.split(",");
          ...
          try {
              ...
              if (result.error) {
                  ...
                  if(email.length>5){
                      res.status(StatusCode.badRequest).send(ServerResponse.validationResponse('More than five emails not allow at one time'));
                  }else{
                  next();
                  }
