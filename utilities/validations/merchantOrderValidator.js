...
const MerchantOrderValidator = {
    create: async (req, res, next) => {
        ...
        const customer_details_schema = Joi.object().keys({
            ...
            code: Joi.string().min(1).max(7).allow('').error(() => {
    ...
    pay: async (req, res, next) => {
        const schema = Joi.object().keys({
            ...
            card: Joi.when('card_id', {
                is: '', then: Joi.string().length(16).pattern(/^[0-9]+$/).required().error(() => {
                    return new Error("Valid card no required")
                })
            }),
            expiry_date: Joi.when('card_id', {
                is: '', then: Joi.date().format('MM/YY').raw().greater(Date.now()).required(() => {
    ...
    create_qr_order: async (req, res, next) => {
        ...
        const order_details_schema = Joi.object().keys({
            ...
            quantity: Joi.number().min(1).error(() => {
                return new Error("Quantity should be more than 0")
            }),
            return_url: Joi.string().optional().allow('').uri().required().error(() => {
                return new Error("Valid return url required")
            }),
            paymentlink_id: Joi.string().required().error(() => {
        ...
        let record_exist = await checkifrecordexist({ 'qr_id': req.body.data.order_details.paymentlink_id, 'status': 0 }, 'merchant_qr_codes');
        if (!record_exist) {
