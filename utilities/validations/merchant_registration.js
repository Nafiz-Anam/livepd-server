    ...
    resend_link: async (req, res, next) => {
        ...
        try {
            ...
            if (result.error) {
                ...
                let email_exits = await checkifrecordexist({ email: req.bodyString('email') }, 'master_super_merchant');
                if (!email_exits) {
                    res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Merchant with email ${req.bodyString('email')} is not exits`));
                } else {
    ...
    forgot_2fa:async(req,res,next)=>{
        ...
        try {
            ...
            if (result.error) {
                ...
                let email_exits = await checkifrecordexist({ email: req.bodyString('email') }, 'master_super_merchant');
                if (!email_exits) {
                    res.status(StatusCode.badRequest).send(ServerResponse.successmsg(`If your account is identified, you will be receiving an email to reset 2fa.`));
                } else {
                    next();
                }
            }
