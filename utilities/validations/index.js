...
const Validator = {
    ...
    forget_password: async (req, res, next) => {
        if (checkEmpty(req.body, ["email", "user"])) {
            const schema = Joi.object().keys({
                ...
                user: Joi.string().required().error(() => {
    ...
    admin_details_update: async (req, res, next) => {
        ...
        if (checkEmpty(req.body, ["name", "designation", "department", "email", "password", "confirm_password", "mobile_no", "role", "user_id"])) {
            ...
            const schema = Joi.object().keys({
                ...
                role: Joi.string().optional().allow('').required().error(() => {
                    return new Error("Valid role required")
                }),
                username: Joi.string().min(5).max(29).required().pattern(new RegExp(/^(?!.*\.\.)(?!.*\.$)[^\W][\w.]{0,29}$/)).messages({
                    "string.pattern.base": "Username can contain alphanumeric characters and/or underscores(_)",
    ...
    country_add: async (req, res, next) => {
        if (checkEmpty(req.body, ["country_name", "country_code", "dial","mobile_no_length","zero_at_first_place"])) {
            const schema = Joi.object().keys({
                ...
                zero_at_first_place: Joi.string().min(2).max(3).required().error(() => {
                    return new Error("Valid Zero at the first place required")
                }),
                is_high_risk: Joi.number().min(0).max(1).required().error(() => {
                    return new Error("Valid high risk value required")
                }),
                is_this_country_zone: Joi.number().min(0).max(1).required().error(() => {
    ...
    country_update: async (req, res, next) => {
        ...
        if (checkEmpty(req.body, ["country_id", "country_name", "country_code", "dial","mobile_no_length","zero_at_first_place"])) {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    let country_name = await checkifrecordexist({ 'country_name': req.bodyString('country_name'), 'id !=': record_id, 'deleted': 0 }, 'country');
                    let country_code = await checkifrecordexist({ 'country_code': req.bodyString('country_code'), 'id !=': record_id, 'deleted': 0 }, 'country');
