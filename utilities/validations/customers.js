                ...
                try {
                    ...
                    if (result.error) {
                        ...
                        if (!customer_exists || pin_is_blank ||!pin_exist) {
                            if (!customer_exists){
                                res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`ID not found`));
            ...
            if (checkEmpty(req.body, ["name","avatar"])) {
                ...
                try {
                    ...
                    if (result.error) {
                        ...
                        let user_exist = await checkifrecordexist({ 'id': req.user.id }, 'customers');
                ...
                try {
                    ...
                    if (result.error) {
                        ...
                        var customers_exists = await checkifrecordexist({id: req.user.id }, 'customers');
                            var check = await checkifrecordexist({ "pin": old_pin, id: req.user.id }, 'customers');
                            if (!check || !customers_exists) {
                        if (!check) {
                            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse("You have entered wrong old pin"));
                        }
        ...
        otp_sent_email: async (req, res, next) => {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    let email = req.bodyString('email');
                    let email_exist = await checkifrecordexist({email: email,id:req.user.id}, 'customers');
                    let table_email_exist = await checkifrecordexist({email: email}, 'customers');
                    var customers_exists = await checkifrecordexist({id: req.user.id }, 'customers');
                    if (email_exist || !customers_exists || table_email_exist) {
                    if (email_exist) {
                            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`You have changed your email ID`)); 
                    }else if (!customers_exists) {
                        res.status(StatusCode.badRequest).send(ServerResponse.validationResponse("Customer not found"));
                    }else if (table_email_exist) {
                        res.status(StatusCode.badRequest).send(ServerResponse.validationResponse("Email already exist"));
                    }
                    }else {
                        next();
      ...
      console.log(req.body)
            ...
            try {
                ...
                if (result.error) {
                    ...
                    let card_no =  enc_dec.cjs_encrypt(req.bodyString('card_no'));
                    let card_holder_name = req.bodyString('card_holder_name');
                    let card_if_exist = await checkifrecordexist({card_number: card_no}, 'customers_cards');
                    let card_exist = await checkifrecordexist({card_number: card_no,name_on_card:card_holder_name}, 'customers_cards');
                    if (card_exist || card_if_exist ) {
                        if (card_exist) {
                        res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Card number exists`)); 
                        }else if (card_if_exist) {
                            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Card number exists`)); 
                            }
                    } else {
        ...
        card_delete: async (req, res, next) => {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (!card_id_exist) {
                            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Card not found or already deleted`)); 
                    } else {
                        next();
                    }
                }
        ...
        card_hide: async (req, res, next) => {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (!card_id_exist) {
                            res.status(StatusCode.badRequest).send(ServerResponse.validationResponse(`Card not found or already hide`)); 
                    } else {
                        next();
                    }
                }
        ...
        check_mobile_and_code: async (req, res,next) => {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    let  mobile_exist = await checkifrecordexist({dial_code: mobile_code,mobile_no: mobile_no,id:req.user.id}, 'customers');
